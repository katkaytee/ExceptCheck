# Author: Katherine Tan 2018, modified from https://github.com/c2nes/javalang
...
# ---- Helper methods ----
    ...
    def accept(self, *accepts, **optionals):
        last = None
        record_first_position = None
        if 'record_first_position' in optionals:
            record_first_position = optionals['record_first_position']
        ...
        for accept in accepts:
            ...
            last = token
            # Record the position of the first token in the list of accepts
            if accepts.index(accept) == 0 and record_first_position:
                self.recorded_position = last.position
...
# -- Blocks and statements --
    ...
    def parse_block(self):
        ...
        self.accept('}', record_first_position=True)
    ...
    def parse_statement(self):
        ...
        # For ExceptCheck, this section and the relevant function calls make
        # up the area of interest.
        elif self.try_accept('try'):
            resource_specification = None
            block = None
            catches = None
            finally_block = None
            # Keep track of where the try k
            ...
            # Java allows for a resource specification after the try keyword.
...
# -- Try / catch --
    ...
    def parse_catch_clause(self):
        self.accept('catch', '(', record_first_position=True)
        catch_begin = self.recorded_position
        ...
        self.accept(')')
        block = self.parse_block()
        catch_end = self.recorded_position

        return tree.CatchClause(parameter=catch_parameter,
                                block=block,
                                begin_position=catch_begin,
                                end_position=catch_end)
